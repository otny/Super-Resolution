...
class checkpoint():
        def __init__(self, args):
                self.args = args
                self.ok = True  # What???, OK??
                self.log = torch.Tensor()
                now = datetime.datetime.now().strftime('%Y-%m-%d-%H:%M:%S')

                if args.load == '.':
                        if args.save == '.': args.save = now
                        self.dir = './logs' + args.save         # 保存するディレクトリ path 作成
                else:
                        self.dir = './logs' + args.load         # ロードするディレクトリ path 作成
                        if not os.path.exists(self.dir):        # ロードファイルが見つからなかった場合
                                args.load = '.'
                        else:                                   # ロードファイルが見つかった場合
